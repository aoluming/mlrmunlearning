...
class Cocoun_model(Qwen2_5_VLForConditionalGeneration):
    def __init__(
        ...
        unlearn_lm_loss_weight: float = 0.0000001, # 新增: 控制梯度上升部分的权重
    ):
    ...
    def forward(
        ...
        **kwargs
        ...
        # 2. 定义 Mask 区域
        # ==========================================================
        # 找到第一个 <latent_start> 的位置
        # 假设你的推理链是: <think> <latent_start> <latent>...<latent_end> Answer
        is_latent_start = (input_ids == self.start_latent_id)
        is_latent_end = (input_ids == self.end_latent_id)
        
        # 逻辑：从第一个 latent_start 开始到结束，全部纳入 Unlearn 范围
        # 这样可以确保中间的 latent tokens 不会被“学习”
        unlearn_region_mask = is_latent_start.cumsum(dim=1) > 0
        
        # 定义 Answer 区域 (仅用于精准 GA)
        answer_region_mask = (is_latent_end.cumsum(dim=1) - is_latent_end.long()) > 0
        ...
        shift_logits = logits[..., :-1, :].contiguous()
        shift_labels = labels[..., 1:].contiguous()
        shift_unlearn_mask = unlearn_region_mask[..., 1:].contiguous()
        shift_answer_mask = answer_region_mask[..., 1:].contiguous()
        ...
        # --- [A] Answer 部分的梯度上升 (GA) ---
        # 这是为了让模型“不敢”说出那个名字
        ga_mask = shift_answer_mask.view(-1) & (flat_labels != -100)
        if torch.any(ga_mask):
            ga_loss = per_token_loss[ga_mask].mean()
            # 这里的减号实现 GA
            total_loss = (ga_loss * self.unlearn_lm_loss_weight)

        # --- [B] Latent 区域的隐式遗忘 (Contrastive) ---
        # 找到所有 <latent> token 的位置
        latent_pos_mask = (input_ids == self.latent_token_id)
        latent_states = last_hidden_state[latent_pos_mask]
        
        unlearning_contrastive_loss = torch.tensor(0.0, device=logits.device)
        if latent_states.shape[0] > 0 and self.forget_concept_vector is not None:
            ...
            # 我们希望相似度越小越好（甚至推向负数/正交）
            # 这里的相似度是一个 -1 到 1 的值
            cos_sim = (latent_norm * concept_norm).sum(dim=-1)
            
            # 使用 MSE 将相似度推向 0 (正交) 或者直接最小化 cos_sim
            unlearning_contrastive_loss = torch.mean(cos_sim) 
            total_loss += (self.unlearning_loss_weight * unlearning_contrastive_loss)
            # total_loss = (self.unlearning_loss_weight * unlearning_contrastive_loss)

        # 注意：这里我们完全删除了对 Latent Token 的 learn_loss (Positive CE)
        # 因为我们不希望模型在看到敏感图片时产生任何稳定的、被期待的 latent 表示
        # total_loss=torch.tensor(0.0, device=logits.device)
        if self.training:
            print(f">>> GA_Loss: {-ga_loss.item() if torch.any(ga_mask) else 0:.4f} | "
                  f"Cos_Sim: {unlearning_contrastive_loss.item():.4f}")
    ...
    def generate(
        ...
        **kwargs
        ...
        # 4. 构造返回格式（为了适配推理脚本，返回 batch 维度的 tensor）
